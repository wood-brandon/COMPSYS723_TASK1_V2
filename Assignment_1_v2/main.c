...
void PushButtonISR() {
	...
	xSemaphoreGiveFromISR(LoadQ_sem, 5);
}
...
void SwitchPollingTask(void* pvParameters)
{
	...
	while (1)
	{
		...
		sw_result = sw_result & 0x1F; // mask only bottom 5 switches
		// get switch values
		for (i = 0; i < NUM_LOADS; i++) {
			j[i] = (bool)(sw_result & (1 << i));
		}

		xSemaphoreTake(sys_status_flag, portMAX_DELAY); //loadstates semaphore
		//if load shedding
...
void VGATask(void* pvParameters)
{
	...
	while (1)
	{
		// critical prevents freqValue update during writing
		taskENTER_CRITICAL();
		startpos = freq_index;
		int i;
...
void drawMaintenace() {
	if (managementState) {
		alt_up_pixel_buffer_dma_draw_box(pixel_buf, 352, 365, 362, 375, 0x3ff, 0); //Green
	}
	else {
		alt_up_pixel_buffer_dma_draw_box(pixel_buf, 352, 365, 362, 375, ((0x3ff << 20) + (0x3ff << 10) + (0x3ff)), 0); //Green
	}
}
...
void LoadControlTask(void* pvParameters)
{
	...
	while (1)
	{
		if (xSemaphoreTake(LoadQ_sem, portMAX_DELAY)) {
			GREEN_LED = 0;
			RED_LED = 0;

			// ensure management mode turns off all loads with flag
			// check disable_shed instead of management mode to only run once
			if (disable_shed) {
				LoadReconnect("All");
				disable_shed = false;
			}

			// format states for writing to led gpio
			for (i = 0; i < NUM_LOADS; i++) {
				switch (LoadStates[i]) {
				case(LOAD_STATE_ON):
					RED_LED += (1 << i);
		...
		if (first_shed) {
			first_shed = false;
			time = (long)(alt_timestamp()); //timer is restarted each load, so start_time = 0
